...
"$TLS" server "$TMP" A 127.0.0.1 "$PORT" & PID=$!; sleep .3; "$TLS" client "$TMP" 127.0.0.1 "$PORT" neta-lab.local 3 >/dev/null; echo 'NETA-LAB-022 phase=baseline identity=A'
"$ROOT/common/linux/accept_observed_baseline.sh" "$PORT" true
kill "$PID"; wait "$PID" 2>/dev/null || true
"$TLS" server "$TMP" B 127.0.0.1 "$PORT" & PID=$!; sleep .3; "$TLS" client "$TMP" 127.0.0.1 "$PORT" neta-lab.local 2 >/dev/null; echo 'NETA-LAB-022 phase=validation identity=B'
