...
cleanup(){ set +e; [[ -n ${CLIENT_PID:-} ]] && kill "$CLIENT_PID" 2>/dev/null; [[ -n ${SERVER_PID:-} ]] && kill "$SERVER_PID" 2>/dev/null; ip netns del "$NS" 2>/dev/null; }
...
sleep .3
BASELINE_HOLD_SECONDS=${NETA_BASELINE_HOLD_SECONDS:-8}
python3 "$ROOT/common/client/tcp_lab_client.py" 10.203.0.2 "$PORT" --connections 1 --upload-bytes 8388608 --hold-seconds "$BASELINE_HOLD_SECONDS" --scenario "NETA-LAB-netem-$MODE-baseline" & CLIENT_PID=$!
"$ROOT/common/linux/accept_observed_baseline.sh" "$PORT" false 10.203.0.2
...
IMPAIRED_UPLOAD_BYTES=8388608
IMPAIRED_HOLD_SECONDS=3
if [[ $MODE == loss ]]; then IMPAIRED_UPLOAD_BYTES=33554432; IMPAIRED_HOLD_SECONDS=5; fi
python3 "$ROOT/common/client/tcp_lab_client.py" 10.203.0.2 "$PORT" --connections 1 --upload-bytes "$IMPAIRED_UPLOAD_BYTES" --hold-seconds "$IMPAIRED_HOLD_SECONDS" --scenario "NETA-LAB-netem-$MODE-impaired"
wait "$SERVER_PID"
