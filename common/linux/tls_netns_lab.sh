...
cleanup(){ set +e; [[ -n ${CLIENT_PID:-} ]] && kill "$CLIENT_PID" 2>/dev/null; [[ -n ${PID:-} ]] && kill "$PID" 2>/dev/null; ip netns del "$NS" 2>/dev/null; rm -rf "$TMP"; }; trap cleanup EXIT INT TERM
...
"$TLS" client "$TMP" 10.204.0.2 "$PORT" neta-lab.local "${NETA_BASELINE_HOLD_SECONDS:-8}" >"$TMP/baseline-client.log" 2>&1 & CLIENT_PID=$!
echo "$SCENARIO phase=baseline identity=A"
"$ROOT/common/linux/accept_observed_baseline.sh" "$PORT" true 10.204.0.2
